...
const Adverts: React.FC<AdvertsProps> = ({}) => {
  ...
  async function fetchData(params: string, object: string) {
    ...
    setdata((prev: any) => {
      return {
        ...prev,
        [object]: response.data.adverts.map((item: any) => (
          <ProductCard
            key={item.id}
            data={{
              slug: item.slug,
    ...
    <div className={styles.adverts}>
      <ul className={styles.links}>
        {links.map((item) => (
          <li
            key={item.title}
            onClick={() => setlink(item.id)}
            style={item.id === link ? { color: "#E61C23" } : {}}
          >
            {item.title + " (" + (data[item.id]?.length || 0) + ")"}
          </li>
        ))}
      </ul>
