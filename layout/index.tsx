    ...
    <>
      <header className={styles.header + ' wrapper'}>
        ...
        <div >
          ...
          <div className={styles.headerRIght} onMouseLeave={() => setIsOpen(false)}>
            <ReactSelectUI width="77px" />

            <div className={styles.headerRIghtChild}>
              <span {...buttonProps}><UserLoginSVG /></span>
              <span
                onClick={() => Router.push('/dashboard')}
                onMouseOver={() => setIsOpen(true)}>
                  {(user?.name?.slice(0, 6) || user?.tel)}</span>
              {
                user?.tel ?
                  <div className={isOpen ? styles['visible'] : styles['hidden']} role='menu'>

                    <Link href={{ pathname: '/dashboard', query: { param: 'Elanlar' } }}>Elanlarım </Link>
                    <Link href={{ pathname: '/dashboard', query: { param: 'Ödənişlər' } }}>Balans artımı </Link>
                    <a href="" onClick={() => localStorage.removeItem('agent')}>Çıxış</a>
                  </div>
